...
module.exports = class ModelS {
  ...
  constructor(row) {
    ...
    this.wheel_type = row.wheel_type;
    this.interior = row.interior;
  }

  // CRUD-------------------------------------

  static async insert({ title, descript, model, color, wheel_type, interior }) {
    const { rows } = await pool.query(
      'INSERT INTO model_s (title, descript, model, color, wheel_type, interior) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [title, descript, model, color, wheel_type, interior]
    );
  ...
  static asyn update(id, {title, descript, model, color, wheel_type, interior}) {
    const { rows } = await pool.query(
      'UPDATE model_s
      SET title=$1,
          descript=$2,
      ...
      RETURNING *
      ',
      [title, descript, model, color, wheel_type, interior, id]
    );
