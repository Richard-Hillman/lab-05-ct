...
module.exports = class ModelThree {
  id;
  title;
  descript;
  model;
  ...
  static async insert({ title, descript, model, color, wheelType, interior }) {
    const { rows } = await pool.query(
      'INSERT INTO model_three(title, descript, model, color, wheel_type, interior) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [title, descript, model, color, wheelType, interior]
    );

    return new ModelThree(rows[0]);
  }

  // ------------------------------------------

  static async find() {
    const { rows } = await pool.query('SELECT * FROM model_three');

    return rows.map(row => new ModelThree(row));
  }
  ...
  static async findById(id) {
    const { rows } = await pool.query(
      'SELECT * FROM model_three WHERE id=$1',
      [id]
    );
  ...
  static async delete(id) {
    const { rows } = await pool.query(
      'DELETE FROM model_three WHERE id=$1 RETURNING *',
      [id]
    );
